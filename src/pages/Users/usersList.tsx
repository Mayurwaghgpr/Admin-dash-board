...
function UsersList() {
  ...
  useEffect(() => {
    axios.get<User[]>(`${import.meta.env.VITE_BASE_URL}/api/users`).then((response) => {
   ...
   const HandelEditUser = () => {
    ...
    axios.put<User>(`${import.meta.env.VITE_BASE_URL}/api/users/${EditedUser?.id}`, EditedUser).then((response) => {
...
setUsers((prev) =>
  ...
  const deleteUser = (id: number) => {
    axios.delete(`${import.meta.env.VITE_BASE_URL}/api/users/${id}`).then(() => {
      filteruser(id)
    });
  };
  ...
  const addnewUser = (newuser: User) => {
    axios.post<User>(`${import.meta.env.VITE_BASE_URL}/api/users`, newuser).then((response) => {
